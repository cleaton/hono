...
export class Node {
  ...
  insert(
    ...
    if (pattern) {
      ...
      node = this.children.get(regexpStr)
      if (!node) {
        if (
          Array.from(this.children.keys()).some(
        ...
        node = new Node()
        this.children.set(regexpStr, node)
        if (name !== '') {
  ...
  buildRegExpStr(): string {
    const childKeys = Array.from(this.children.keys()).sort(compareKey)

    const strList = childKeys.map((k) => {
      const c = this.children.get(k)!
